...
export default class UserDrop extends React.Component {
  ...
  componentDidMount() {
    ...
    const { loading, profileBase64 } = userBasicInfo;
  ...
  componentWillReceiveProps(nextProps) {
    const { authUserInfo } = nextProps;
    const { userBasicInfo, dispatch } = this.props;
  ...
  getOtherUsers = () => {
    ...
    FetchQueryOtherUsers({
      ...
      const arr = records.map((it) => {
        const { roleName } = it;
        const arrroleName = arrroleName ? JSON.parse(roleName) : '';
  ...
  getDropbox = (dispatch, userBasicInfo, authUserInfo, otherusers = [], photos) => {
    const { allotherusers, photo = '' } = this.state;
      ...
      >
        <Row>
          <Col span={24} className="dis-fx">
            {/* <div className="m-panel-img" style={{ paddingLeft: '0.583rem' }}>
              <Avator photoPng={photo} dispatch={dispatch} avatorPng={avatorPng} avatorShowPic={showPic} userBasicInfo={userBasicInfo} forceRender={this.forceRender} />
            </div> */}
  ...
  render() {
    ...
    const { visibleM, otherusers, total, loading, photos, photo } = this.state;
