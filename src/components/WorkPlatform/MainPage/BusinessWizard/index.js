...
class BusinessWizard extends React.Component {
  ...
  handleProStartUrl = (itemValue) => {
    const { stepsData = [], targetUrl } = this.state;
    let datas = [];
  ...
  render() {
    ...
    const { tableData = [], stepsData = [], pagination, pagination: { current = 1, total }, loading, visible, targetUrl, prodSelectList } = this.state;
    const modalProps = {
      title: '流程发起',
      width: '100rem',
      visible,
